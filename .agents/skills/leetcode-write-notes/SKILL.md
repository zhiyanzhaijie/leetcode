...
10. Read `<repo-root>/web/src/solutions/tag_groups.md` and decide whether the problem belongs to a common, reusable algorithmic pattern represented there. If it does, add the completed problem to the matching subgroup. If this is the first problem for a genuinely useful pattern, add a concise English group or subgroup and continue the existing numeric sequence. Do not create tags for problem-specific details or duplicate a problem already listed elsewhere under the same pattern.
11. Verify the final Markdown structurally: exact examples retained, fences balanced, source code copied faithfully under My Solution, classic code contains no `unwrap`, relative asset links remain valid, tag-group numbering remains consistent, and no unsupported correctness claim remains.
