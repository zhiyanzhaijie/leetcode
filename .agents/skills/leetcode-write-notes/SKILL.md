...
6. Select classic solutions from established algorithm knowledge. Default to exactly one: the best-known, clearest approach using the most appropriate data structure. Include additional classic solutions only when they teach a genuinely different reusable idea or the user asks for them. Give My Solution and every Classic Solution its own explanation: the subproblem solved at each step, its key invariant or correctness argument, why the chosen structure fits, and its complexity. Never leave a solution as code without an explanation.
...
8. Evaluate visualization separately for My Solution and for every Classic Solution. Explain each solution with the clearest compact representation available, in this order of preference: a useful visualization, a node/flow representation, then structured logical bullets. Choose Mermaid only when it is the best renderer for the relationship; also consider a Markdown table, ASCII layout, or short state list. For state transitions, boundary/invariant evolution, pointer movement, or algorithm phases, show an official example when it materially clarifies the maintained state. Do not force a diagram when it merely restates the code. Never generate or overwrite SVG/image files unless explicitly requested.
...
10. Read `<repo-root>/web/src/solutions/tag_groups.md` and build a tag-coverage checklist before editing it. Derive candidates from the fetched official `topicTags` and every reusable technique actually used by My Solution or the classic solution. For each candidate, decide whether it is a common reusable pattern or only a generic data domain/implementation detail. Register the problem under every applicable reusable pattern, not only the first matching group. If a pattern has no suitable group, add a concise English group and continue the existing numeric sequence. Prefer a flat top-level list for a new or small category; add a subgroup only when multiple entries share a stable, useful distinction. Do not create tags for problem-specific details or duplicate a problem within the same pattern.
11. Verify the final Markdown structurally: exact examples retained, fences balanced, source code copied faithfully under My Solution, classic code contains no `unwrap`, relative asset links remain valid, and no unsupported correctness claim remains. Treat solution explanation as a completion gate: confirm My Solution and every Classic Solution each contain an approach, an invariant or correctness argument, and a complexity analysis. Treat tag coverage as a completion gate: confirm every reusable candidate in the checklist is registered in `tag_groups.md`, every excluded candidate has an explicit reason, and tag-group numbering remains consistent.
